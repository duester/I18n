...
class I18n private[plural] (private val map : Map[Language, Map[PluralCategory, String]]) {
  def apply(language : Language)(number : Double, parameters : Any*) : Option[String] = {
    val exactCategory = language.category(number)

    val baseStringOpt = extract(language, exactCategory)
    baseStringOpt match {
      case None => None
      case Some(baseString) =>
        val unpackedParametersOpt = parameters.map {
          case i18 : I18n => i18.extract(language, exactCategory)
          case p @ _      => Option(p)
        }
  ...
  private def extract(language : Language, exactCategory : Exact) : Option[String] = {
  ...
  private def parentList[T <: { val next : T }](startItem : T, stopItem : T) : List[T] = {
    @tailrec
  ...
  def |(otherI18 : I18n) : I18n = {
    ...
    new I18n(buildMap(otherI18.map.iterator.toList, this.map))
  }
...
object I18n {
  private[plural] def fromStringInterpolator[L <: Language, C <: PluralCategory : L#AcceptableCategory](sc : StringContext, args : Any*)(language : L, category : C) : I18n = {
  ...
  private def create[L <: Language, C <: PluralCategory](language : L, category : C)(text : String) : I18n = {
